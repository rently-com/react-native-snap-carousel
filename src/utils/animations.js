...
export function stackAnimatedStyles (index, animatedValue, carouselProps, cardOffset) {
  ...
  const getTranslateFromScale = (cardIndex, scale) => {
      const centerFactor = 1 / scale * cardIndex,
        centeredPosition = -Math.round(sizeRef * centerFactor),
        edgeAlignment = Math.round((sizeRef - (sizeRef * scale)) / 2),
        offset = Math.round(cardOffset * Math.abs(cardIndex) / scale);
    ...
    opacityOutputRange = carouselProps.inactiveSlideOpacity === 1 ? [1, 1, 1, 0] : [1, 0.75, 0.5, 0];
...
export function tinderAnimatedStyles (index, animatedValue, carouselProps, cardOffset) {
  const sizeRef = carouselProps.vertical ? carouselProps.itemHeight : carouselProps.itemWidth,
    mainTranslateProp = carouselProps.vertical ? 'translateY' : 'translateX',
    secondaryTranslateProp = carouselProps.vertical ? 'translateX' : 'translateY',

    card1Scale = 0.96,
    card2Scale = 0.92,
    card3Scale = 0.88,

    peekingCardsOpacity = IS_ANDROID ? 0.92 : 1;
  ...
  const getMainTranslateFromScale = (cardIndex, scale) => {
    ...
    getSecondaryTranslateFromScale = (cardIndex, scale) => Math.round(cardOffset * Math.abs(cardIndex) / scale);
