...
const IS_IOS = Platform.OS === 'ios',
   ...
   */
  AnimatedFlatList = FlatList ? Animated.createAnimatedComponent(FlatList) : null,
...
export default class Carousel extends Component {
  ...
  static propTypes = {
    ...
    activeSlideAlignment: PropTypes.oneOf(['center', 'end', 'start']), activeSlideOffset: PropTypes.number, apparitionDelay: PropTypes.number, autoplay: PropTypes.bool, autoplayDelay: PropTypes.number,
  ...
  componentDidMount () {
    const { apparitionDelay, autoplay, firstItem } = this.props,
      _firstItem = this._getFirstItem(firstItem),
      apparitionCallback = () => {
  ...
  componentDidUpdate (prevProps) {
    const { interpolators } = this.state,
      { firstItem, itemHeight, itemWidth, scrollEnabled, sliderHeight, sliderWidth } = this.props,
      itemsLength = this._getCustomDataLength(this.props);
    ...
    const hasNewSliderWidth = sliderWidth && sliderWidth !== prevProps.sliderWidth,
      hasNewSliderHeight = sliderHeight && sliderHeight !== prevProps.sliderHeight,
      hasNewItemWidth = itemWidth && itemWidth !== prevProps.itemWidth,
      hasNewItemHeight = itemHeight && itemHeight !== prevProps.itemHeight,
      hasNewScrollEnabled = scrollEnabled !== prevProps.scrollEnabled;
  ...
  _shouldAnimateSlides (props = this.props) {
    ...
    return inactiveSlideOpacity < 1 ||
            ...
            Boolean(scrollInterpolator) ||
            Boolean(slideInterpolatedStyle) ||
  ...
  _shouldUseCustomAnimation () {
    ...
    return Boolean(activeAnimationOptions) && !this._shouldUseStackLayout() && !this._shouldUseTinderLayout();
  }
  ...
  _getCustomData (props = this.props) {
    ...
    if (loopClonesPerSide > dataLength) {
      const dataMultiplier = Math.floor(loopClonesPerSide / dataLength),
        remainder = loopClonesPerSide % dataLength;
    ...
    if (index >= dataLength + loopClonesPerSide) {
      ...
      if (loopClonesPerSide > dataLength) {
        const baseDataIndexes = [],
          dataIndexes = [],
          dataMultiplier = Math.floor(loopClonesPerSide / dataLength),
          remainder = loopClonesPerSide % dataLength;
  ...
  _getFirstItem (index, props = this.props) {
    const { loopClonesPerSide } = props,
      itemsLength = this._getCustomDataLength(props);
  ...
  _getActiveItem (offset) {
    const { activeSlideOffset, swipeThreshold } = this.props,
      center = this._getCenter(offset),
      centerOffset = activeSlideOffset || swipeThreshold;
  ...
  _initPositionsAndInterpolators (props = this.props) {
    const { data, itemWidth, itemHeight, scrollInterpolator, vertical } = props,
      sizeRef = vertical ? itemHeight : itemWidth;
  ...
  _getSlideAnimation (index, toValue) {
    const { interpolators } = this.state,
      ...
      animatedValue = interpolators && interpolators[index];
  ...
  _playCustomSlideAnimation (current, next) {
    const { interpolators } = this.state,
      itemsLength = this._getCustomDataLength(),
      _currentIndex = this._getCustomIndex(current),
      _currentDataIndex = this._getDataIndex(_currentIndex),
      _nextIndex = this._getCustomIndex(next),
      _nextDataIndex = this._getDataIndex(_nextIndex),
      animations = [];
  ...
  _hackActiveSlideAnimation (index, goTo, force = false) {
    ...
    const itemsLength = data && data.length,
      direction = goTo || itemsLength === 1 ? 'start' : 'end';
  ...
  _scrollTo (offset, animated = true) {
    const { vertical } = this.props,
      wrappedRef = this._getWrappedRef();
  ...
  _onScroll (event) {
    const { callbackOffsetMargin, enableMomentum, onScroll } = this.props,
      ...
      scrollOffset = event ? this._getScrollOffset(event) : this._currentContentOffset,
      nextActiveItem = this._getActiveItem(scrollOffset),
      itemReached = nextActiveItem === this._itemToSnapTo,
      scrollConditions =
  ...
  _renderItem ({ item, index }) {
    ...
    const animate = this._shouldAnimateSlides(),
      Component = animate ? Animated.View : View,
      animatedStyle = animate ? this._getSlideInterpolatedStyle(index, animatedValue) : {},
      ...
      mainDimension = vertical ? { height: itemHeight } : { width: itemWidth },
      specificProps = this._needsScrollView() ? {
  ...
  _getComponentOverridableProps () {
    const {
      ...
      visibleItems = Math.ceil(vertical ?
        sliderHeight / itemHeight :
        sliderWidth / itemWidth) + 1,
      initialNumPerSide = this._enableLoop() ? loopClonesPerSide : 2,
      initialNumToRender = visibleItems + (initialNumPerSide * 2),
      maxToRenderPerBatch = 1 + (initialNumToRender * 2),
